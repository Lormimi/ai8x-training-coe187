...
class ZeroesvsOnes(Dataset):
    """
    Zeroes vs Ones dataset <https://www.kaggle.com/datasets/salader/ones-vs-zeroes> Dataset.
    ...
    label_to_folder_map = {'zero': 'zeroes', 'one': 'ones'}

    def __init__(self, root_dir, d_type, transform=None,
                 resize_size=(128, 128), augment_data=False):
        self.root_dir = root_dir
        self.data_dir = os.path.join(root_dir, 'zeroes_vs_ones', d_type)

        if not self.__check_zeroesvsones_data_exist():
    ...
    def __check_zeroesvsones_data_exist(self):
    ...
    def __print_download_manual(self):
        ...
        print("Download the dataset to the \'data\' folder by visiting this link: "
              "\'https://www.kaggle.com/datasets/salader/ones-vs-zeroes\'")
        print("If you do not have a Kaggle account, sign up first.")
        print("Unzip the downloaded file and find \'test\' and \'train\' folders "
              "and copy them into \'data/zeroes_vs_ones\'. ")
        print("Make sure that images are in the following directory structure:")
        print("  \'data/zeroes_vs_ones/train/zeroes\'")
        print("  \'data/zeroes_vs_ones/train/ones\'")
        print("  \'data/zeroes_vs_ones/test/zeroes\'")
        print("  \'data/zeroes_vs_ones/test/ones\'")
...
def get_zeroesvsones_dataset(data, load_train, load_test):
    """
    Load the Zeroes vs Ones dataset.
    ...
    if load_train:
        train_dataset = ZeroesvsOnes(root_dir=data_dir, d_type='train',
                                   transform=transform, augment_data=True)
    else:
        train_dataset = None

    if load_test:
        test_dataset = ZeroesvsOnes(root_dir=data_dir, d_type='test', transform=transform)
    else:
        test_dataset = None
...
datasets = [
    {
        ...
        'input': (3, 128, 128),
        'output': ('zero', 'one'),
        'loader': get_zeroesvsones_dataset,
    },
]
